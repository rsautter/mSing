...
def NMDFA(timeseries: np.ndarray, lag: np.ndarray,q: np.ndarray = 2) -> Tuple[np.array, np.ndarray]:
    ...
    for li,i in enumerate(lag):
        ...
        print()
        print("Ub[0]", np.array(ub)[0])
        print("Ub shape:",np.array(ub).shape)
        print("Var (ub):",np.var(np.array(ub),axis=0).shape)
